...
func GRPCClientMiddleware(remoteServiceName string) middleware.Middleware {
	router := composite.DefaultComposite()
	multi.Register(router)
	lane := router.Lane()
	return func(handler middleware.Handler) middleware.Handler {
		return func(ctx context.Context, req interface{}) (reply interface{}, err error) {
			info, _ := grpc.FromClientContext(ctx)
			api := info.FullMethod
			ctx = startClientContext(ctx, remoteServiceName, lane, api)
			ctx = startClientSpan(ctx, api)
...
func startClientSpan(ctx context.Context, api string) context.Context {
